    ...
    if (!mifare_port) {
        navigator.serial.requestPort({ filters: [{ usbVendorId: 0x0403 }] }).then(port => {
            ...
            port.open({ baudRate: 9600, baudrate: 9600 }).then(() => {
                ...
                $('.rs232-scanner').off('click').removeClass('text-dark text-info text-danger text-success text-warning').addClass('text-success').css('cursor', '').attr('title', i18n("scannerConnected")).show();
            });
            navigator.serial.ondisconnect = () => {
                $('.rs232-scanner').off('click').removeClass('text-dark text-info text-danger text-success text-warning').addClass('text-danger').css('cursor', '').attr('title', i18n("scannerDisConnected")).show();
            };
        }).catch(() => {
            mifare_port = null;
        });
...
setTimeout(() => {
    if (!('serial' in navigator) && !NFC) {
        $('.rs232-scanner').off('click').removeClass('text-info text-danger text-muted text-warning').addClass('text-warning').css('cursor', '').attr('title', i18n("scannerUnavailable")).show();
    }
}, 500);
