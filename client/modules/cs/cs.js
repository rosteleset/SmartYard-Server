({
    ...
    init: function () {
        if (parseInt(myself.uid) > 0) {
            if (AVAIL("cs", "sheets") && AVAIL("tt", "tt") && modules.mqtt) {
                this.menuItem = leftSide("fas fa-fw fa-table", i18n("cs.csSheet"), "?#cs", "tt");
            }
        }
                                    ...
                                    for (let k in rs) {
                                        if (rs[k] == row || start >= 0) {
                                            ...
                                            if (k - start < cells) {
                                                ...
                                                if (closed) {
                                                    modules.cs.issuesInSheet[uid] += `<span class="nowrap noselect border-no-shadow csIssueHighlight ${modules.cs.currentSheet.sheet.issueClosedClass}"><a class="cs-issue-span pointer pl-1 pr-1" href="?#tt.issue&issue=${r.issues.issues[i].issueId}" title="${title}">${r.issues.issues[i].issueId}</a></span>`;
                                                } else
                                                if (installers && installers.length && !done) {
                                                    modules.cs.issuesInSheet[uid] += `<span class="nowrap noselect border-no-shadow csIssueHighlight ${modules.cs.currentSheet.sheet.issueAssignedClass}"><a class="cs-issue-span pointer pl-1 pr-1" href="?#tt.issue&issue=${r.issues.issues[i].issueId}" title="${title}">${r.issues.issues[i].issueId}</a></span>`;
                                                } else
                                                if ((!installers || !installers.length) && done) {
                                                    modules.cs.issuesInSheet[uid] += `<span class="nowrap noselect border-no-shadow csIssueHighlight ${modules.cs.currentSheet.sheet.issueDoneClass}"><a class="cs-issue-span pointer pl-1 pr-1" href="?#tt.issue&issue=${r.issues.issues[i].issueId}" title="${title}">${r.issues.issues[i].issueId}</a></span>`;
                                                } else
                                                if (installers && installers.length && done) {
    ...
    route: function (params) {
        ...
        document.title = i18n("windowTitle") + " :: " + i18n("cs.csSheet");
