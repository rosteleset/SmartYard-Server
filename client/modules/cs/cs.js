                                    ...
                                    for (let k in rs) {
                                        if (rs[k] == row || start >= 0) {
                                            ...
                                            if (k - start < cells) {
                                                ...
                                                if (closed) {
                                                    modules.cs.issuesInSheet[uid] += `<span class="nowrap border-no-shadow csIssueHighlight ${modules.cs.currentSheet.sheet.issueClosedClass}"><a class="cs-issue-span pointer pl-1 pr-1" href="?#tt&issue=${r.issues.issues[i].issueId}" title="${title}">${r.issues.issues[i].issueId}</a></span><br />`;
                                                } else
                                                if (installers && installers.length && !done) {
                                                    modules.cs.issuesInSheet[uid] += `<span class="nowrap border-no-shadow csIssueHighlight ${modules.cs.currentSheet.sheet.issueAssignedClass}"><a class="cs-issue-span pointer pl-1 pr-1" href="?#tt&issue=${r.issues.issues[i].issueId}" title="${title}">${r.issues.issues[i].issueId}</a></span><br />`;
                                                } else
                                                if ((!installers || !installers.length) && done) {
                                                    modules.cs.issuesInSheet[uid] += `<span class="nowrap border-no-shadow csIssueHighlight ${modules.cs.currentSheet.sheet.issueDoneClass}"><a class="cs-issue-span pointer pl-1 pr-1" href="?#tt&issue=${r.issues.issues[i].issueId}" title="${title}">${r.issues.issues[i].issueId}</a></span><br />`;
                                                } else
                                                if (installers && installers.length && done) {
                                                    modules.cs.issuesInSheet[uid] += `<span class="nowrap border-no-shadow csIssueHighlight ${modules.cs.currentSheet.sheet.issueAssignedClass} ${modules.cs.currentSheet.sheet.issueDoneClass}"><a class="cs-issue-span pointer pl-1 pr-1" href="?#tt&issue=${r.issues.issues[i].issueId}" title="${title}">${r.issues.issues[i].issueId}</a></span><br />`;
                                                } else {
                                                    modules.cs.issuesInSheet[uid] += `<a class="nowrap border-no-shadow cs-issue-span pointer text-dark pl-1 pr-1" href="?#tt&issue=${r.issues.issues[i].issueId}" title="${title}">${r.issues.issues[i].issueId}</a><br />`;
                                                }
                                                modules.cs.issuesInSheet[uid] += "</div>";
                                            }
                                        }
