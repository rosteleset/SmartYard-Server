    ...
    renderIssue: function (issue) {
        ...
        if (!isEmpty(issue.issue.tags)) {
            ...
            for (let i in issue.issue.tags) {
                ...
                t += `<span class="mr-1 text-bold" style='border: solid thin #cbccce; padding-left: 7px; padding-right: 7px; padding-top: 2px; padding-bottom: 2px; color: ${fg}; border-radius: 4px; background: ${bg};'><i class="fas fa-tag mr-2"></i>${issue.issue.tags[i]}</span>`;
            }
            h += $.trim(t);
            h += "</div>";
            h += "</td>";
