...
CREATE TABLE users (uid serial not null primary key, login character varying not null, password character varying not null, enabled integer, real_name character varying, e_mail character varying, phone character varying);
...
CREATE TABLE api_methods(aid character varying not null primary key, api character varying not null, method character varying not null, request_method character varying not null);
CREATE UNIQUE INDEX api_methods_uniq on api_methods(api, method, request_method);
CREATE TABLE users_rights(uid integer not null, aid character varying not null, allow integer);
CREATE UNIQUE INDEX users_rights_uniq on users_rights(uid, aid);
CREATE TABLE groups_rights(gid integer not null, aid character varying not null, allow integer);
CREATE UNIQUE INDEX groups_rights_uniq on groups_rights(gid, aid);
CREATE TABLE api_methods_common(aid character varying not null primary key);
CREATE TABLE api_methods_personal(aid character varying not null primary key);
