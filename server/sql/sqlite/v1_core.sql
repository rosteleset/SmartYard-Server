...
-- users
CREATE TABLE core_users (uid integer not null primary key autoincrement, login text not null, password text not null, enabled integer, real_name text, e_mail text, phone text, default_route text);
...
CREATE TABLE core_api_methods_by_backend(aid text not null primary key, backend text);
