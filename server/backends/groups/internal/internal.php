    ...
    namespace backends\groups {
        ...
        class internal extends groups {
            ...
            public function getGroups($uid = false) {
                if ($uid === false) {
                    $groups = $this->db->queryEx("select gid, name, acronym, (select count(*) from (select uid from (select uid from core_users_groups g1 where g1.gid=g2.gid union select admin from core_groups g3 where g3.gid=g2.gid union select uid from core_users u1 where u1.primary_group=g2.gid) as t2 group by uid) as t3) as users, admin, login as \"adminLogin\" from core_groups as g2 left join core_users on g2.admin = core_users.uid order by name, acronym, gid");
                } else {
                    if (!checkInt($uid)) {
                        return false;
                    }
