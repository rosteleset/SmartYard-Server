    ...
    namespace backends\groups {
        ...
        class internal extends groups {
            ...
            public function getGroups($uid = false) {
                if ($uid === false) {
                    error_log("select gid, name, acronym, (select count(*) from (select uid from (select uid from core_users_groups g1 where g1.gid=g2.gid union (select admin from core_groups g3 where g3.gid=g2.gid) union (select uid from core_users u1 where u1.primary_group=g2.gid)) users, admin, login as \"adminLogin\" from core_groups as g2 left join core_users on core_groups.admin = core_users.uid order by gid");
