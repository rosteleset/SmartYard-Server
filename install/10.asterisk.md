## Install Asterisk LTS v22

1. Install

```bash
cd /tmp && \
wget https://downloads.asterisk.org/pub/telephony/asterisk/asterisk-22-current.tar.gz -O - | gzip -dc | tar -xvf - && \
cd asterisk-22*
```
...
cp /opt/rbt/asterisk/logger.sample.conf /opt/rbt/asterisk/logger.conf
```

### Optional: SIP TLS for mobile clients

To accept mobile registrations over TLS (and enable SRTP for those endpoints), use the TLS sample instead of the plain PJSIP sample, place certificate and key under `/etc/asterisk/keys/`, then set a numeric `sip_tls_port` on the Asterisk entry in `server/config/config.json`.

```bash
cp /opt/rbt/asterisk/pjsip.tls-sample.conf /opt/rbt/asterisk/pjsip.conf
mkdir -p /etc/asterisk/keys
# put asterisk.crt and asterisk.key into /etc/asterisk/keys/
```

In `backends.sip.servers` for the Asterisk server, add for example `"sip_tls_port": 5061`. Without a numeric `sip_tls_port`, behavior stays TCP/UDP as before.
