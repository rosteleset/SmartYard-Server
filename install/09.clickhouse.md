...
If you want to use a non-standard device (for example, Sputnik Cloud),
and your syslog table already exists, you need to add a ```sub_id``` column:
```clickhouse
ALTER TABLE default.syslog
ADD COLUMN sub_id String AFTER ip,
ADD INDEX syslog_sub_id sub_id TYPE set(100) GRANULARITY 1024;
```
