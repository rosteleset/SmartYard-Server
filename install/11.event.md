...
6. Add systemd services by used intercom model:
   ...
   ###### All services
   ````shell
   find /opt/rbt/install/systemd/event/ -name "event_*.service" -exec ln -sf {} /etc/systemd/system/ \;
   ````
   ###### Beward
    ````shell
...
8. Run created systemd services:
    - reload systemctl daemon and enable all new event services::
   ````shell
